...
class StockDatabase:
    ...
    def save_price_history(self, ticker: str, df: pd.DataFrame) -> None:
        ...
        has_date_idx = isinstance(df.index, pd.DatetimeIndex) or df.index.name == 'Date'
        if 'Date' not in df.columns and not has_date_idx:
        ...
        try:
            ...
            for idx, row in df.iterrows():
                date_val = row['Date'] if 'Date' in df.columns else idx
                dt = pd.to_datetime(date_val)
                # Convert timezone-aware datetimes to naive datetimes to match DB format
            ...
            if records:
                # Fetch existing records to prevent IntegrityError
                ...
                for record in records:
                    existing = existing_map.get(record['date'])
                    if existing:
